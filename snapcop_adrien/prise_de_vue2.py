...
for i in range(0, frames):
    ...
    cmd = 'sudo gphoto2 --capture-image-and-download --force-overwrite --filename %s && sudo mv %s %s/%s' % (filename, filename, dossier, serie)
...
cmd = 'cd %s/%s; for i in %s%i*.jpg; do convert -quality 50 -resize 800x600 "$i" "../%s/%s/$i"; done && cp -R ../%s ../.. && rm -rf ../%s' % (dossier, serie, photos_name, shoot_nb, dossier_name, serie, dossier_name, dossier_name)
subprocess.check_output(cmd, shell=True)

#Stack les photos basse qualité pour un rendu rapide
cmd2 = 'enfuse -o %s.jpg --exposure-weight=1 --saturation-weight=0.1 --contrast-weight=1 --exposure-sigma=0 --exposure-mu=1 --gray-projector=l-star --hard-mask %s/%s/*.jpg && mv %s.jpg %s/%s' % (image_fs, dossier_name, serie, image_fs, dossier_name, serie)
                ...
                for evt in evts:
                    if evt.type == pygame.locals.KEYDOWN:
                        if evt.key == 27:
                            ...
                            pass
        final = pygame.image.load(('%s/%s/%s.jpg') % (dossier_name, serie, image_fs))
        indicateur()
