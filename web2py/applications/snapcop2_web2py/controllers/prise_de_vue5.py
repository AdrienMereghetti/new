...
def Config():
    form=FORM(TABLE(TR('Nom du dossier des photos "lourdes" : ',INPUT(_type="text",_name="dossier",value="shoot",requires=IS_NOT_EMPTY())),
                    ...
                    TR('Interval de temps entre deux prise de vue (sec) : ',INPUT(_type="text",_name="interval",value="4",requires=IS_INT_IN_RANGE(0, 20))),
                    ...
                    TR("Voulez-vous un défilement automatique ou manuel des photos ?",SELECT('Veuillez indiquer votre choix', 'Automatique','Manuel',_name="defil",requires=IS_IN_SET(['Automatique','Manuel']))),
